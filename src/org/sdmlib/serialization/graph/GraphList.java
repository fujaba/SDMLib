...
public class GraphList implements BaseEntityList
{
   ...
   {
      GraphEdge edge = new GraphEdge().withSource(
         new GraphNode().withClassName(sourceName)).withTarget(
         new GraphNode().withClassName(targetName));
      addEdge(edge);
      return this;
   }
   ...
   public boolean addEdge(GraphEdge edge)
   {
      for (Iterator<GraphEdge> i = this.edges.iterator(); i.hasNext();)
      {
         ...
         if (item.getSource().has(edge.getTarget().getItems())
            && item.getTarget().has(edge.getSource().getItems()))
         {
            // Back again
