...
public abstract class Entity implements BaseEntity
{
   ...
   public Entity initWithMap(Object value)
   {
      if (value != null && value instanceof Map<?, ?>)
      {
         ...
         for (Iterator<?> i = map.entrySet().iterator(); i.hasNext();)
         {
            ...
            if (item != null)
            {
               this.put(EntityUtil.wrap(mapEntry.getKey(), this).toString(),
                  EntityUtil.wrap(item, this));
            }
         }
      }
      return this;
    ...
    * if there is already an object stored under the key then a EntityList is
    * stored under the key to hold all of the accumulated values. If there is
    * already a EntityList, then the new value is appended to it. In contrast,
   ...
   public Entity addToList(String key, Object value)
   {
      ...
      if (object == null)
      {
         this.put(key, value instanceof EntityList ? getNewArray().put(value)
               : value);
      }
   ...
   /**
    * Put a key/value pair in the Entity, where the value will be a Entity which
         ...
         else
         {
            if (id >= 0 || id == -2)
            {
               if (child instanceof EntityList)
               {
                  ...
                  if (list.size() >= id)
                  {
                     ((Entity) list.get(id)).setValue(key.substring(end + 1),
                        value);
                  }
               }
            }
