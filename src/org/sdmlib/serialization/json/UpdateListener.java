...
public class UpdateListener implements PropertyChangeListener
{
         ...
         if (remove == null && update != null)
         {
            ...
            while (keys.hasNext())
            {
               ...
               if (value == null)
               {
                  ...
                  // ERROR
                  if (!this.map.skipCollision(masterObj, key, value, remove,
                     update))
                  {
                     if (checkPrio(prio))
                     {
         ...
         else if (update == null && remove != null)
         {
            ...
            while (keys.hasNext())
            {
               ...
               else
               {
                  if (checkValue(value, key, remove))
                  {
                     setValue(creator, masterObj, key,
                        creator.getValue(refObject, key), IdMapEncoder.REMOVE);
                  }
                  ...
                  && removeJsonObject instanceof JsonObject)
               {
                  ...
                  this.map.readMessages(key, masterObj, this.map.decode(json),
                     json, IdMapEncoder.REMOVE);
               }
            }
            return true;
         }
   ...
   private boolean checkValue(Object value, String key, JsonObject oldJsonObject)
   {
      if (value != null)
      {
         ...
         {
            // GLAUB ICH MAL
            String oldId = (String) ((JsonObject) oldValue).get(JsonIdMap.ID);
