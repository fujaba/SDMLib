...
import org.sdmlib.test.examples.helloworld.model.util.EdgePO;
...
import org.sdmlib.test.examples.helloworld.model.util.GraphCreator;
...
public class HelloWorldTTC2011 {
   ...
   * <p>
   * Storyboard
   * <a href='./src/test/java/org/sdmlib/test/examples/helloworld/HelloWorldTTC2011.java' type=
   * 'text/x-java'>TTC2011HelloWorldConstantTransformation1</a>
   * </p>
   * <p>
   ...
   * </p>
   * <script> var json = { "typ":"class", "nodes":[ { "typ":"node", "id":"Greeting", "attributes":[
   * "text : String" ] } ], "edges":[ { "typ":"unidirectional", "source":{ "id":"Greeting",
   * "cardinality":"one", "property":"tgt" }, "target":{ "id":"Greeting", "cardinality":"one",
   * "property":"greeting" } } ] } ; new Graph(json,
   * {"canvasid":"canvasTTC2011HelloWorldConstantTransformation1ClassDiagram2", "display":"html",
   ...
   * </pre>
   * 
   * <script> var json = { "type":"object", "nodes":[ { "type":"patternObject", "style":"create",
   * "id":"g1 : GreetingPO", "attributes":[ "<< create>>", "text == Hello World" ] } ], "edges":[] } ;
   * json["options"]={"canvasid":"canvasTTC2011HelloWorldConstantTransformation1PatternDiagram5",
   ...
   * <script> var json = { "type":"object", "nodes":[ { "type":"patternObject", "style":"create",
   * "id":"g1 : GreetingPO", "attributes":[ "<< create>>", "text == Hello World" ] } ], "edges":[] } ;
   * json["options"]={"canvasid":"canvasTTC2011HelloWorldConstantTransformation1PatternDiagram8",
   * "display":"html", "fontsize":10,"bar":true}; var g = new Graph(json); g.layout(100,100);
   * </script>
   * <p>
   * For completeness just the host graph:
   * </p>
   * <script> var json = { "type":"objectdiagram", "nodes":[ { "type":"clazz", "id":"G1 : Greeting",
   * "attributes":[ "greeting=null", "greetingMessage=null", "person=null", "text=Hello World",
   * "tgt=null" ] } ], "edges":null } ;
   * json["options"]={"canvasid":"canvasTTC2011HelloWorldConstantTransformation111", "display":"svg",
   ...
   * 
   * @see <a href=
   *      '../../../../../../../../doc/TTC2011HelloWorldConstantTransformation1.html'>TTC2011HelloWorldConstantTransformation1.html</a>
   */
  // @Test
  public void testTTC2011HelloWorldConstantTransformation1() {
    ...
    greetClass.withUniDirectional(greetClass, "tgt", Association.ONE);
  ...
  public void testTTC2011HelloWorldConstantTransformation2WithReferences() {
    ...
    greetingClazz.withBidirectional(greetingMessageClazz, "greetingMessage", Association.ONE, "greeting", Association.ONE);

    greetingClazz.withBidirectional(personClazz, "person", Association.ONE, "greeting", Association.ONE);
  ...
  public void testTTC2011HelloWorldCountNumberOfNodes() {
    ...
    new Association(nodeClazz).with("nodes").with(Association.MANY)
        .with(new Association(graphClazz).with("graph").with(Association.ONE));

    new Association(edgeClazz).with("edges").with(Association.MANY)
        .with(new Association(graphClazz).with("graph").with(Association.ONE));

    new Association(nodeClazz).with("src").with(Association.ONE)
        .with(new Association(edgeClazz).with("outEdges").with(Association.MANY));

    new Association(nodeClazz).with("tgt").with(Association.ONE)
        .with(new Association(edgeClazz).with("inEdges").with(Association.MANY));
  ...
  public void testTTC2011SimpleMigration() {
    ...
    new Association(graphComponentClazz).with("gcs").with(Association.MANY)
        .with(new Association(graphClazz).with("parent").with(Association.ONE));
    ...
    new Association(nodeClazz).with("copy").with(Association.ONE)
        .with(new Association(nodeClazz).with("orig").with(Association.ONE));
    ...
    graphClazz.withBidirectional(nodeClazz, "nodes", Association.MANY, "graph", Association.ONE);

    nodeClazz.withBidirectional(nodeClazz, "linksTo", Association.MANY, "linksFrom", Association.MANY);
