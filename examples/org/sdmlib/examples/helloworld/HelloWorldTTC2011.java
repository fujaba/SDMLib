...
public class HelloWorldTTC2011
{
   ...
   public void testConstantTransformation1()
   {
      Storyboard storyboard = new Storyboard("examples",
            "TTC2011HelloWorldConstantTransformation1")
      ...
      storyboard.addSVGImage(model.dumpClassDiagram("examples",
         "TTC2011HelloWorldConstantTransformation1ClassDiag"));

      // ==========================================================================

      storyboard
         .add("The code that builds and runs the transformation / pattern looks like: ");
      ...
      storyboard.add(p.dumpDiagram("TTC2011HelloWorldConstantTransformation1",
         false));

      storyboard.assertTrue("Constant transformation has match",
         p.getHasMatch());

      storyboard
         .add("At runtime the object structure for the pattern and for the hostgraph looks like: ");

      storyboard
         .add(p
            .dumpDiagram("TTC2011HelloWorldConstantTransformation1AfterTransformation"));
      ...
      storyboard
         .add("TTC2011HelloWorldConstantTransformation1 number of matches is "
            + noOfMatches);

      // ==========================================================================

      storyboard
         .add("For fairness, the java code that does this transformation looks like: ");
      storyboard.markCodeStart();
      Greeting greeting = new Greeting().withText("Hello World");
      storyboard.addCode("examples");
   ...
   {
      Storyboard storyboard = new Storyboard("examples",
            "TTC2011HelloWorldConstantTransformation2WithReferences")
      ...
      Clazz greetingMessageClazz = new Clazz(
            "org.sdmlib.examples.helloworld.GreetingMessage").withAttribute(
         "text", "String");

      Clazz personClazz = new Clazz("org.sdmlib.examples.helloworld.Person")
         .withAttribute("name", "String");

      new Association().withTarget(greetingMessageClazz, "greetingMessage",
         R.ONE).withSource(greetingClazz, "greeting", R.ONE);

      new Association().withTarget(personClazz, "person", R.ONE).withSource(
         greetingClazz, "greeting", R.ONE);
      ...
      storyboard.addSVGImage(model.dumpClassDiagram("examples",
         "TTC2011HelloWorldConstantTransformation2ClassDiag"));
      ...
      PersonPO personPO = greetingPO.hasPerson().hasName("TTC Participants");
      storyboard.addCode("examples");

      storyboard.add(p.dumpDiagram("TTC2011HelloWorldConstantTransformation2",
         false));
      ...
      storyboard
         .add(p
            .dumpDiagram("TTC2011HelloWorldConstantTransformation2AfterTransformation"));
      ...
      Person person = greeting.createPerson().withName("TTC Participants");
      storyboard.addCode("examples");
   ...
   {
      Storyboard storyboard = new Storyboard("examples",
            "TTC2011HelloWorldModelToText")
         .with("For model to text transformation we provide a simple template mechanism. ");

      storyboard
         .add("The model transformation that builds our object model looks like: ");
      storyboard.markCodeStart();
      ModelPattern p = new ModelPattern().startCreate();
      ...
      storyboard.addObjectDiagram(p.getJsonIdMap(),
         greetingPO.getCurrentMatch());

      storyboard
         .add("The model to text transfromation template mechanism is used like this: ");

      storyboard.markCodeStart();
      StringBuilder text = new StringBuilder("message name");

      CGUtil.replaceAll(text, "message", greetingMessagePO.getText(), "name",
         personPO.getName());
      ...
      storyboard.markCodeStart();
      CGUtil.find("Hi dudes", 0, "message name", "message", greetingMessagePO,
         GreetingMessage.PROPERTY_TEXT, "name", personPO, Person.PROPERTY_NAME);
      storyboard.addCode("examples");
      ...
      storyboard
         .add("Alternatively, plain java code that does the model to text transformation looks like: ");
   ...
   public void testTTC2011HelloWorldCountNumberOfNodes()
   {
      ...
      Clazz edgeClazz = new Clazz("Edge").withAttribute("name", "String");

      Clazz nodeClazz = new Clazz("Node").withAttribute("name", "String");

      new Association().withTarget(nodeClazz, "nodes", R.MANY).withSource(
         graphClazz, "graph", R.ONE);

      new Association().withTarget(edgeClazz, "edges", R.MANY).withSource(
         graphClazz, "graph", R.ONE);

      new Association().withTarget(nodeClazz, "src", R.ONE).withSource(
         edgeClazz, "outEdges", R.MANY);

      new Association().withTarget(nodeClazz, "tgt", R.ONE).withSource(
         edgeClazz, "inEdges", R.MANY);
      ...
      storyboard.addSVGImage(model.dumpClassDiagram("examples",
         "TTC2011HelloWorldSimpleGraphClassDiag"));
      ...
      storyboard
         .addObjectDiagram(CreatorCreator.createIdMap("hg"), graph, true);
      ...
      storyboard.add(storyboard.getMethodText("examples", this.getClass()
         .getName(), "countNodesInJava(Graph)"));
      ...
      storyboard.add(storyboard.getMethodText("examples", this.getClass()
         .getName(), "countNodesPerPattern(Graph)"));

      // storyboard.addObjectDiag(p.getJsonIdMap(),
      // p.getElements().iterator().next());

      storyboard.add(ModelPattern.lastPattern.dumpDiagram(
         "countNodesPerPatternRule", false));

      storyboard.add(ModelPattern.lastPattern
         .dumpDiagram("countNodesPerPatternRuleWithMatch"));
      ...
      storyboard
         .add("Retrieve set of matching nodes per pattern and count its elements: ");

      countNodesPerNodeSet(graph);

      storyboard.add(storyboard.getMethodText("examples", this.getClass()
         .getName(), "countNodesPerNodeSet(Graph)"));

      storyboard.add(ModelPattern.lastPattern.dumpDiagram(
         "countNodesPerNodeSet", false));

      storyboard.add(ModelPattern.lastPattern
         .dumpDiagram("countNodesPerNodeSetWithMatch"));
      ...
      storyboard.add("<hr/>");
      storyboard
         .add("<h2>Retrieve set of looping edges per pattern and count its elements: </h2>");

      countLoopingEdgesPerPattern(graph);

      storyboard.add(storyboard.getMethodText("examples", this.getClass()
         .getName(), "countLoopingEdgesPerPattern(Graph)"));

      storyboard.add(ModelPattern.lastPattern.dumpDiagram(
         "countLoopingEdgesPerPattern", false));

      storyboard.add(ModelPattern.lastPattern
         .dumpDiagram("countLoopingEdgesPerPatternWithMatch"));
      ...
      storyboard.add(storyboard.getMethodText("examples", this.getClass()
         .getName(), "countLoopingEdgesInJava(Graph)"));
      ...
      storyboard.add(storyboard.getMethodText("examples", this.getClass()
         .getName(), "countIsolatedNodesPerPattern(Graph)"));

      storyboard.add(ModelPattern.lastPattern.dumpDiagram(
         "countIsolatedNodesPerPattern", false));

      storyboard.add(ModelPattern.lastPattern.dumpDiagram(
         "countIsolatedNodesPerPatternWithMatch", true));
      ...
      storyboard.add(storyboard.getMethodText("examples", this.getClass()
         .getName(), "countIsolatedNodesInJava(Graph)"));
      ...
      storyboard.add(storyboard.getMethodText("examples", this.getClass()
         .getName(), "countCirclesOfThreeNodesPerPattern(Graph)"));

      storyboard.add(ModelPattern.lastPattern.dumpDiagram(
         "countCirclesOfThreeNodesPerPattern", false));

      storyboard.add(ModelPattern.lastPattern.dumpDiagram(
         "countCirclesOfThreeNodesPerPatternWithMatch", true));

      storyboard.add("systemout: " + systemout);

      storyboard
         .add("If you want to print the matched nodes for each circle, you need to iterate through the matches: ");

      countCirclesOfThreeNodesPerPatternReportMatches(graph);

      storyboard.add(storyboard.getMethodText("examples", this.getClass()
         .getName(), "countCirclesOfThreeNodesPerPatternReportMatches(Graph)"));

      storyboard.add(ModelPattern.lastPattern.dumpDiagram(
         "countCirclesOfThreeNodesPerPatternReportMatches", false));

      storyboard.add(ModelPattern.lastPattern.dumpDiagram(
         "countCirclesOfThreeNodesPerPatternReportMatchesWithMatch", true));
      ...
      storyboard.add(storyboard.getMethodText("examples", this.getClass()
         .getName(), "countCirclesOfThreeNodesInJava(Graph)"));
      ...
      storyboard.add(storyboard.getMethodText("examples", this.getClass()
         .getName(), "countDanglingEdgesPerPattern(Graph)"));

      storyboard.add(ModelPattern.lastPattern.dumpDiagram(
         "countDanglingEdgesPerPattern", false));

      storyboard.add(ModelPattern.lastPattern.dumpDiagram(
         "countDanglingEdgesPerPatternWithMatch", true));
      ...
      storyboard.add(storyboard.getMethodText("examples", this.getClass()
         .getName(), "countDanglingEdgesInJava(Graph)"));
   ...
   public void testTTC2011HelloWorldReverseEdges()
   {
      ...
      storyboard.add(storyboard.getMethodText("examples", this.getClass()
         .getName(), "reverseEdgesPerPattern(Graph)"));

      storyboard.add(ModelPattern.lastPattern.dumpDiagram(
         "reverseEdgesPerPattern", false));

      storyboard.add(ModelPattern.lastPattern.dumpDiagram(
         "reverseEdgesPerPatternWithMatch", true));
      ...
      storyboard.add(storyboard.getMethodText("examples", this.getClass()
         .getName(), "reverseEdgesInJava(Graph)"));
   ...
   public void testTTC2011SimpleMigration()
   {
      ...
      storyboard.addSVGImage(model.dumpClassDiagram("examples",
         "TTC2011HelloWorldSimpleMigrationSourceClassDiag"));
      ...
      Clazz graphComponentClazz = new Clazz(
            "org.sdmlib.examples.helloworld.GraphComponent").withAttribute(
         "text", "String");
      ...
      new Association().withTarget(graphComponentClazz, "gcs", R.MANY)
      ...
      new Association().withTarget(nodeClazz, "copy", R.ONE).withSource(
         nodeClazz, "orig", R.ONE);
      ...
      storyboard.add(storyboard.getMethodText("examples", this.getClass()
         .getName(), "simpleMigrationPerPattern(Graph,Storyboard)"));
      ...
      storyboard.add(storyboard.getMethodText("examples", this.getClass()
         .getName(), "simpleMigrationInJava(Graph)"));
      ...
      storyboard.add(storyboard.getMethodText("examples", this.getClass()
         .getName(), "simpleMigrationByCloning(Graph)"));
      ...
      storyboard.add(storyboard.getMethodText("examples", this.getClass()
         .getName(), "simpleMigrationByJsonArray(Graph)"));
      ...
      storyboard.add(storyboard.getMethodText("examples", this.getClass()
         .getName(), "simpleMigrationByGenericGraph(Graph,Storyboard)"));
      ...
      new Association().withTarget(nodeClazz, "linksTo", R.MANY).withSource(
         nodeClazz, "linksFrom", R.MANY);

      model.generate("examples", "examples");

      storyboard.addSVGImage(model.dumpClassDiagram("examples",
         "TTC2011HelloWorldSimpleMigrationEvenMoreEvolvedDiag"));
      ...
      storyboard
         .add(storyboard
            .getMethodText("examples", this.getClass().getName(),
               "simpleMigrationToEvenMoreEvolvedGraphByGenericGraph(Graph,Storyboard)"));
   ...
   public void testTTC2011DeleteNodeWithSpecificName()
   {
      ...
      storyboard.add(storyboard.getMethodText("examples", this.getClass()
         .getName(), "deleteNodeWithNameN1(Graph)"));

      deleteNodeWithNameN1(graph);

      storyboard.add(ModelPattern.lastPattern.dumpDiagram(
         "deleteNodeWithNameN1_nodeN1", false));
   ...
   public void testTTC2011InsertTransitiveEdges()
   {
      ...
      storyboard.add(storyboard.getMethodText("examples", this.getClass()
         .getName(), "insertTransitiveEdges(Graph)"));

      storyboard.add(ModelPattern.lastPattern.dumpDiagram(
         "insertTransitiveEdges", false));

      storyboard.add("Transformation with matches:");

      storyboard.add(ModelPattern.lastPattern.dumpDiagram(
         "insertTransitiveEdgesWithMatch", true));

      storyboard.add("We have inserted " + noOfMatches
         + " new transitive edges. Result graph:");
   ...
   private int insertTransitiveEdges(Graph graph)
   {
      ...
      GraphPO graphPO = p.hasElementGraphPO(graph).withPatternObjectName(
         "graphPO2");
   ...
   {
      // find node
      NodePO nodeN1PO = new ModelPattern().hasElementGraphPO(graph).hasNodes()
         .hasName("n1");

      // destroy all leaving edges
      nodeN1PO.startSubPattern().hasOutEdges().destroy().endSubPattern()
         .allMatches();

      // destroy all incoming edges
      nodeN1PO.startSubPattern().hasInEdges().destroy().endSubPattern()
         .allMatches();
   ...
   private Graph simpleMigrationToEvenMoreEvolvedGraphByGenericGraph(
         Graph origGraph, Storyboard storyboard)
   {
      GenericGraph genGraph = new Specific2Generic().convert(
         CreatorCreator.createIdMap("g1"), origGraph);

      // rename name to text attributes
      new org.sdmlib.models.objects.creators.ModelPattern()
         .hasElementGenericGraphPO(genGraph).hasObjects().hasAttrs()
         .hasName(Node.PROPERTY_NAME).startCreate().hasName(Node.PROPERTY_TEXT)
         .allMatches();

      storyboard
         .add(org.sdmlib.models.objects.creators.ModelPattern.lastPattern
            .dumpDiagram(
               "simpleMigrationToEvenMoreEvolvedGraphByGenericGraph_renameAttr",
               false));

      // replace n.l.e.l.n by n.l.n
      GenericObjectPO edgePO = new org.sdmlib.models.objects.creators.ModelPattern()
         .hasElementGenericGraphPO(genGraph).hasObjects()
         .hasType(Edge.class.getName());

      GenericLinkPO srcLinkPO = edgePO.hasOutgoingLinks().hasTgtLabel(
         Edge.PROPERTY_SRC);

      GenericLinkPO tgtLinkPO = edgePO.hasOutgoingLinks().hasTgtLabel(
         Edge.PROPERTY_TGT);
      ...
      GenericLinkSet allMatches = tgtLinkPO.startCreate().hasSrc(srcNodePO)
         .hasSrcLabel(Node.PROPERTY_LINKSFROM)
         .hasTgtLabel(Node.PROPERTY_LINKSTO).allMatches();

      storyboard
         .add(org.sdmlib.models.objects.creators.ModelPattern.lastPattern
            .dumpDiagram(
               "simpleMigrationToEvenMoreEvolvedGraphByGenericGraph_replaceEdges",
               false));
      ...
      storyboard
         .add(org.sdmlib.models.objects.creators.ModelPattern.lastPattern
            .dumpDiagram(
               "simpleMigrationToEvenMoreEvolvedGraphByGenericGraph_removeDanglingEdges",
               false));

      Graph tgtGraph = (Graph) new Generic2Specific().convert(
         CreatorCreator.createIdMap("tg"), null, genGraph);
         ...
         Storyboard storyboard)
   {
      ...
      storyboard.addObjectDiagramWith(genGraph.getObjects(),
         genGraph.getLinks(), genGraph.getObjects().getAttrs());
      ...
      storyboard
         .add(org.sdmlib.models.objects.creators.ModelPattern.lastPattern
            .dumpDiagram("simpleMigrationByGenericGraph_renameAttr", false));

      // storyboard.add("<hr/>");

      // rename graph--nodes links to parent--gcs links
      new org.sdmlib.models.objects.creators.ModelPattern()
         .hasElementGenericGraphPO(genGraph).hasLinks()
         .hasTgtLabel(Node.PROPERTY_GRAPH).startCreate()
         .hasTgtLabel(Node.PROPERTY_PARENT).hasSrcLabel(Graph.PROPERTY_GCS)
         .allMatches();

      storyboard
         .add(org.sdmlib.models.objects.creators.ModelPattern.lastPattern
            .dumpDiagram("simpleMigrationByGenericGraph_renameLink1", false));
      ...
      new org.sdmlib.models.objects.creators.ModelPattern()
         .hasElementGenericGraphPO(genGraph).hasLinks()
         .hasSrcLabel(Node.PROPERTY_GRAPH).startCreate()
         .hasSrcLabel(Node.PROPERTY_PARENT).hasTgtLabel(Graph.PROPERTY_GCS)
         .allMatches();

      storyboard
         .add(org.sdmlib.models.objects.creators.ModelPattern.lastPattern
            .dumpDiagram("simpleMigrationByGenericGraph_renameLink2", false));
   ...
   private Graph simpleMigrationByJsonArray(Graph graph)
   {
      ...
      for (int i = 0; i < jsonArray.size(); i++)
      {
         ...
         {
            JsonObject jsonProps = jsonObject
               .getJsonObject(JsonIdMap.JSON_PROPS);

            // migrate nodes and edges to gcs
            JsonArray gcsArray = jsonProps.getJsonArray(Graph.PROPERTY_NODES);

            gcsArray.addAll(Arrays.asList(jsonProps.getJsonArray(
               Graph.PROPERTY_EDGES).toArray()));
         ...
         else if (Node.class.getName().equals(className))
         {
            ...
            jsonProps
               .put(Node.PROPERTY_TEXT, jsonProps.get(Node.PROPERTY_NAME));

            jsonProps.remove(Node.PROPERTY_NAME);

            // migrate graph to parent
            jsonProps.put(Node.PROPERTY_PARENT,
               jsonProps.get(Node.PROPERTY_GRAPH));
      ...
      Graph copyGraph = (Graph) CreatorCreator.createIdMap("hg").decode(
         jsonArray);
   ...
   private void simpleMigrationInJava(Graph origGraph)
   {
      ...
      if (origGraph != null)
      {
         ...
         {
            Node copyNode = (Node) copyGraph.createGcsNode().withOrig(origNode)
         ...
         {
            Edge copyEdge = (Edge) copyGraph.createGcsEdge().withText(
               origEdge.getName());
   ...
   private void simpleMigrationPerPattern(Graph graph, Storyboard storyboard)
   {
      ...
      storyboard.add(migrateGraph.dumpDiagram(
         "simpleMigrationPerPattern_migrateGraph", false));
      ...
      tgtGraphPO = migrateNodesPattern.hasElementGraphPO(tgtGraphPO
         .getCurrentMatch());
      ...
      storyboard.add(migrateNodesPattern.dumpDiagram(
         "simpleMigrationPerPattern_migrateNodesPattern", false));
      ...
      tgtGraphPO = migrateEdgesPattern.hasElementGraphPO(tgtGraphPO
         .getCurrentMatch());
      ...
      while (migrateEdgesPattern.getHasMatch())
      {
         ...
         NodePO copySrcNodePO = migrateSrcHalfLinkPattern
            .hasElementEdgePO(srcEdgePO.getCurrentMatch()).hasSrc().hasCopy();

         EdgePO copyEdgePO = migrateSrcHalfLinkPattern
            .hasElementEdgePO(tgtEdgePO.getCurrentMatch());
         ...
         NodePO copyTgtNodePO = migrateTgtHalfLinkPattern
            .hasElementEdgePO(srcEdgePO.getCurrentMatch()).hasTgt().hasCopy();

         EdgePO copyEdgePO2 = migrateTgtHalfLinkPattern
            .hasElementEdgePO(tgtEdgePO.getCurrentMatch());
      ...
      storyboard.add(migrateEdgesPattern.dumpDiagram(
         "simpleMigrationPerPattern_migrateEdgesPattern", false));

      storyboard.add(migrateSrcHalfLinkPattern.dumpDiagram(
         "simpleMigrationPerPattern_migrateSrcHalfLinkPattern", false));

      storyboard.add(migrateTgtHalfLinkPattern.dumpDiagram(
         "simpleMigrationPerPattern_migrateTgtHalfLinkPattern", false));
   }
   ...
   {
      NodeSet allMatches = new ModelPattern().hasElementGraphPO(graph)
         .hasNodes().allMatches();

      systemout = "Nodes: " + allMatches.getName().concat(", ")
         + " Number of nodes: " + allMatches.size();
   }

   public void countLoopingEdgesPerPattern(Graph graph)
   {
      EdgePO edgesPO = new ModelPattern().hasElementGraphPO(graph).hasEdges();
      ...
      systemout = "Looping Edges: " + loopingEdges.getName().concat(", ")
         + " Number of looping edges: " + loopingEdges.size();
   }
   ...
   public void countLoopingEdgesInJava(Graph graph)
   {
      ...
      systemout = sysout.substring(0, sysout.length() - 2)
         + " Number of looping edges: " + noOfLoopingEdges;
   }
   ...
   public ModelPattern countIsolatedNodesPerPattern(Graph graph)
   {
      ...
      systemout = "Isolated nodes: " + isolatedNodes.getName().concat(", ")
         + " Number of isolated nodes: " + isolatedNodes.size();
   ...
   public void countIsolatedNodesInJava(Graph graph)
   {
      ...
      systemout = systemout.substring(0, systemout.length() - 2)
         + " Number of isolated nodes: " + noOfIsolatedNodes;
   }
   ...
   public ModelPattern countCirclesOfThreeNodesPerPatternReportMatches(
         Graph graph)
   {
      ModelPattern p = new ModelPattern();
         ...
         for (Edge firstEdge : firstNode.getOutEdges())
         {
            ...
            if (secondNode != null && secondNode != firstNode)
            {
               for (Edge secondEdge : secondNode.getOutEdges())
               {
                  ...
                  if (thirdNode != null && thirdNode != secondNode
                     && thirdNode != firstNode)
                  {
   ...
   public ModelPattern countDanglingEdgesPerPattern(Graph graph)
   {
      ...
      systemout = "Dangling edges " + allMatches.getName().concat(", ")
         + " number: " + allMatches.size();
   ...
   public void countDanglingEdgesInJava(Graph graph)
   {
      ...
      systemout = systemout.substring(0, systemout.length() - 2) + " number: "
         + noOfMatches;
   }
