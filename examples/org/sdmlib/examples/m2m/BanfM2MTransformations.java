...
public class BanfM2MTransformations
{
   ...
   public void testBanfM2MTransformation()
   {
      ...
      Clazz edgeClazz = new Clazz("Relation").withAttribute("kind", "String");

      new Association().withTarget(nodeClazz, "persons", R.MANY).withSource(
         graphClazz, "graph", R.ONE);

      new Association().withTarget(edgeClazz, "relations", R.MANY).withSource(
         graphClazz, "graph", R.ONE);
      ...
      Clazz graphComponentClazz = new Clazz("GraphComponent").withAttribute(
         "text", "String");

      nodeClazz = new Clazz("Person").withSuperClass(graphComponentClazz);

      edgeClazz = new Clazz("Relation").withSuperClass(graphComponentClazz);
      ...
      nodeClazz = new Clazz("Person").withAttribute("text", "String");

      new Association().withTarget(nodeClazz, "persons", R.MANY).withSource(
         graphClazz, "graph", R.ONE);

      new Association().withTarget(nodeClazz, "knows", R.MANY).withSource(
         nodeClazz, "knows", R.MANY);
      ...
      Graph tgtGraph2 = simpleMigrationToEvenMoreEvolvedGraphByGenericGraph(
         graph, storyboard);
      ...
      storyboard.add("<hr/>");
      storyboard
         .add("Let us derive the reverse transformation for the first model evolution:");

      Graph sourceGraphRevers = simpleReverseMigration(tgtGraph, storyboard);

      storyboard.addObjectDiagramWith(sourceGraphRevers.getPersons(),
         sourceGraphRevers.getRelations());

      storyboard.addLogEntry(R.DONE, "zuendorf", "10.10.2013 10:10:42", 20, 0,
         "should be published in paper");
   ...
   private Graph simpleReverseMigration(Graph tgtGraph, Storyboard storyboard)
   {
      ...
      GenericGraph genGraph = new Specific2Generic().convert(
         GraphComponentCreator.createIdMap("s"), tgtGraph);
      ...
      renameFirstNameAttrRule.hasElementGenericGraphPO(null).hasObjects()
         .hasType(Person.class.getName()).hasAttrs()
         .hasName(Person.PROPERTY_FIRSTNAME).startCreate()
         .hasName(GraphComponent.PROPERTY_TEXT).allMatches();

      storyboard.markCodeStart();
      Pattern reverseRenameFirstNameAttrRule = revertRule(renameFirstNameAttrRule);
      storyboard.addCode();

      storyboard.add(renameFirstNameAttrRule.dumpDiagram(
         "banfSimpleMigrationRenameFirstNameAttrRule2", false));

      storyboard.add(reverseRenameFirstNameAttrRule.dumpDiagram(
         "banfSimpleMigrationReverseRenameFirstNameAttrRule", false));

      storyboard.add("<hr/>");

      renameKindAttrRule = new org.sdmlib.models.objects.creators.ModelPattern();
      renameKindAttrRule.hasElementGenericGraphPO(genGraph).hasObjects()
         .hasType(Relation.class.getName()).hasAttrs()
         .hasName(Relation.PROPERTY_KIND).startCreate()
         .hasName(GraphComponent.PROPERTY_TEXT).allMatches();
      ...
      storyboard.add(renameKindAttrRule.dumpDiagram(
         "banfSimpleMigrationRenameKindAttrRule2", false));

      storyboard.add(reverseRenameKindAttrRule.dumpDiagram(
         "banfSimpleMigrationReverseRenameKindAttrRule", false));

      storyboard.add("<hr/>");

      // rename graph--nodes links to parent--gcs links
      renamePersonsLinkRule = new org.sdmlib.models.objects.creators.ModelPattern();
      renamePersonsLinkRule.hasElementGenericGraphPO(null).hasLinks()
         .hasTgtLabel(Person.PROPERTY_GRAPH)
         .hasSrcLabel(Graph.PROPERTY_PERSONS).startCreate()
         .hasTgtLabel(GraphComponent.PROPERTY_PARENT)
         .hasSrcLabel(Graph.PROPERTY_GCS).allMatches();

      storyboard.markCodeStart();
      Pattern reverseRenamePersonsLinkRule = revertRule(renamePersonsLinkRule);
      storyboard.addCode();

      storyboard.add(renamePersonsLinkRule.dumpDiagram(
         "banfSimpleMigrationRenamePersonsLinkRule2", false));

      storyboard.add(reverseRenamePersonsLinkRule.dumpDiagram(
         "banfSimpleMigrationReverseRenamePersonsLinkRule", false));

      storyboard.add("<hr/>");

      // rename graph--edges links to parent--gcs links
      renameRelationsLinkRule = new org.sdmlib.models.objects.creators.ModelPattern();
      renameRelationsLinkRule.hasElementGenericGraphPO(null).hasLinks()
         .hasSrcLabel(Relation.PROPERTY_GRAPH)
         .hasTgtLabel(Graph.PROPERTY_RELATIONS).startCreate()
         .hasSrcLabel(GraphComponent.PROPERTY_PARENT)
         .hasTgtLabel(Graph.PROPERTY_GCS).allMatches();

      storyboard.markCodeStart();
      Pattern reverseRenameRelationsLinkRule = revertRule(renameRelationsLinkRule);
      storyboard.addCode();

      storyboard.add(renameRelationsLinkRule.dumpDiagram(
         "banfSimpleMigrationRenameRelationsLinkRule2", false));

      storyboard.add(reverseRenameRelationsLinkRule.dumpDiagram(
         "banfSimpleMigrationReverseRenameRelationsLinkRule", false));

      storyboard.add("<hr/>");

      // apply the transformations
      PatternObject boundPO = (PatternObject) reverseRenameRelationsLinkRule
         .getElements().first();
      reverseRenameRelationsLinkRule.rebind(boundPO, genGraph);
      reverseRenameRelationsLinkRule.allMatches();

      boundPO = (PatternObject) reverseRenamePersonsLinkRule.getElements()
         .first();
      ...
      boundPO = (PatternObject) reverseRenameFirstNameAttrRule.getElements()
         .first();
      ...
      Graph srcGraph = (Graph) new Generic2Specific().convert(
         GraphCreator.createIdMap("s"), null, genGraph);
      ...
      for (PatternElement<Object> elem : backwardRule.getElements())
      {
         if (elem instanceof AttributeConstraint)
         {
            ...
            if (Pattern.CREATE.equals(attrConstraint.getModifier()))
            {
               ...
               AttributeConstraint otherConstraint = attrConstraint.getSrc()
                  .getAttrConstraints()
                  .hasAttrName(attrConstraint.getAttrName()).first();
   ...
   {
      GenericGraph genGraph = new Specific2Generic().convert(
         PersonCreator.createIdMap("g1"), origGraph);
      ...
      new org.sdmlib.models.objects.creators.ModelPattern()
         ...
         .hasName(Person.PROPERTY_FIRSTNAME).startCreate()
         .hasName(Person.PROPERTY_TEXT).allMatches();

      storyboard
         .add(org.sdmlib.models.objects.creators.ModelPattern.lastPattern
            .dumpDiagram(
               "banfsimpleMigrationToEvenMoreEvolvedGraphByGenericGraph_renameAttr",
               false));
      ...
      GenericLinkSet allMatches = tgtLinkPO.startCreate().hasSrc(srcNodePO)
         .hasSrcLabel(Person.PROPERTY_KNOWS).hasTgtLabel(Person.PROPERTY_KNOWS)
         .allMatches();

      storyboard
         .add(org.sdmlib.models.objects.creators.ModelPattern.lastPattern
            .dumpDiagram(
               "banfsimpleMigrationToEvenMoreEvolvedGraphByGenericGraph_replaceEdges",
               false));
      ...
      storyboard
         .add(org.sdmlib.models.objects.creators.ModelPattern.lastPattern
            .dumpDiagram(
               "banfsimpleMigrationToEvenMoreEvolvedGraphByGenericGraph_removeDanglingEdges",
               false));

      Graph tgtGraph = (Graph) new Generic2Specific().convert(
         PersonCreator.createIdMap("tg"), null, genGraph);
   ...
   {
      GenericGraph genGraph = new Specific2Generic().convert(
         GraphCreator.createIdMap("g"), origGraph);

      storyboard.addObjectDiagramWith(genGraph.getObjects(),
         genGraph.getLinks(), genGraph.getObjects().getAttrs());

      renameFirstNameAttrRule = new org.sdmlib.models.objects.creators.ModelPattern();
      renameFirstNameAttrRule.hasElementGenericGraphPO(genGraph).hasObjects()
         .hasAttrs().hasName(Person.PROPERTY_FIRSTNAME).startCreate()
         .hasName(GraphComponent.PROPERTY_TEXT).allMatches();

      storyboard.add(renameFirstNameAttrRule.dumpDiagram(
         "banfsimpleMigrationByGenericGraph_renameAttr", false));

      // rename name to text attributes
      renameKindAttrRule = new org.sdmlib.models.objects.creators.ModelPattern();
      renameKindAttrRule.hasElementGenericGraphPO(genGraph).hasObjects()
         .hasAttrs().hasName(Relation.PROPERTY_KIND).startCreate()
         .hasName(GraphComponent.PROPERTY_TEXT).allMatches();

      storyboard
         .add(org.sdmlib.models.objects.creators.ModelPattern.lastPattern
            .dumpDiagram("banfsimpleMigrationByGenericGraph_renameAttr2", false));

      // rename graph--nodes links to parent--gcs links
      new org.sdmlib.models.objects.creators.ModelPattern()
         .hasElementGenericGraphPO(genGraph).hasLinks()
         .hasTgtLabel(Person.PROPERTY_GRAPH).startCreate()
         .hasTgtLabel(GraphComponent.PROPERTY_PARENT)
         .hasSrcLabel(Graph.PROPERTY_GCS).allMatches();

      storyboard
         .add(org.sdmlib.models.objects.creators.ModelPattern.lastPattern
            .dumpDiagram("banfsimpleMigrationByGenericGraph_renameLink1", false));
      ...
      new org.sdmlib.models.objects.creators.ModelPattern()
         .hasElementGenericGraphPO(genGraph).hasLinks()
         .hasSrcLabel(Relation.PROPERTY_GRAPH).startCreate()
         .hasSrcLabel(GraphComponent.PROPERTY_PARENT)
         .hasTgtLabel(Graph.PROPERTY_GCS).allMatches();

      storyboard
         .add(org.sdmlib.models.objects.creators.ModelPattern.lastPattern
            .dumpDiagram("banfsimpleMigrationByGenericGraph_renameLink2", false));

      storyboard.addObjectDiagramWith(genGraph.getObjects(),
         genGraph.getLinks(), genGraph.getObjects().getAttrs());

      Graph tgtGraph = (Graph) new Generic2Specific().convert(
         GraphComponentCreator.createIdMap("tg"), null, genGraph);
