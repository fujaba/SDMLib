...
public class TaskFlowModel
{
   ...
   public void taskFlowModel()
   {
      ...
      model.createClazz("PeerProxy", "ip", STRING, "port", "int", "idMap",
         SDMLibJsonIdMap.class.getName());

      model.createClazz("SocketThread", "ip", STRING, "port", "int", "idMap",
         SDMLibJsonIdMap.class.getName(), "defaultTargetThread", "Object");

      model
         .createClazz("FetchFileFlow", "fileServer", PeerProxy.class.getName(),
            "idMap", SDMLibJsonIdMap.class.getName(), "fileName",
            String.class.getSimpleName()).withSuperClass(taskFlowClass)
         .createMethods("run()", "void");

      Clazz loggerClazz = model.createClazz("Logger", "startPeer",
         PeerProxy.class.getName()).withSuperClass(taskFlowClass);

      Clazz logEntryClass = loggerClazz.createClassAndAssoc("LogEntry",
         "entries", MANY, "logger", ONE);
      logEntryClass.withAssoc(logEntryClass, "children", MANY, "parent", ONE);
      logEntryClass.withAttributes("nodeName", STRING, "taskName", STRING);

      Clazz timerClass = model.createClazz(Timer.class.getName()).withExternal(
         true);

      model.createClazz("SDMTimer").withSuperClass(timerClass)
         .withMethods(new Method("schedule(TimerTask)", "void"));

      storyboard.addSVGImage(model.dumpClassDiagram("swtsrc",
         "taskflowmodeldiag"));
